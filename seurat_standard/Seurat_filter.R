...
load_data <- function(){
  ...
  if (visr.param.workflow == "single"){
    if (visr.param.Import_method == "load_raw"){
      ...
      path1 <- paste(path_to_outs,"outs/filtered_gene_bc_matrices",sep="/")
      path2 <- paste(path_to_outs,"outs/filtered_gene_bc_matrices_mex",sep="/") #aggr
      ...
      if (dir.exists(path1)){
        ...
        visr.message("Cannot find \"filtered_gene_bc_matrices\" or \"filtered_gene_bc_matrices_mex\"")
      }
  ...
  if (is.null(gbmData@meta.data$lib_id)){
    gbmData@meta.data$lib_id <- get_lib_id(gbmData@cell.names)
  }
  return(gbmData)
}
