...
This report shows the results of clustering and differential expression analysis using **Seurat**. The parameters used are the default parameters in the **Seurat** app in **VisR**. 
...
## Filter Dataset
```{r Load Data, results='hide', out.width=400}
# Load Data ---------------------------------------------------------------
...
## Find Variable Genes
```{r Find variable Genes, results='hide'}
...
```{r calculate number of PCs, results='hide'}
auto_num_PC <- calculate_nPC(gbmData)
```

### Elbow Method
```{r Elbow, results='hide',fig.width=6}
gbmData <- draw_elbow(gbmData)
```

In this report, PC1 through PC`r auto_num_PC` are considered as sigificant.

### Jackstraw
```{r Jackstraw, results='hide', fig.height=10, fig.width=15}
...
### PC Heatmap
```{r PC Heatmap, results='hide', fig.width=15,fig.height=20}
...
```{r Clustering Cells, results='hide'}
gbmData <- cluster_cells(gbmData, "pca")
```

## Differential Expression Analysis {.tabset}

The heatmap below shows the expression of the top 2 DE genes (ordered by adjusted p-value) for each cluster. The table below shows the top 50 DE genes for each cluster. The complete list of DE can be found in the output file ``r DE_output``.
...
* ``var_gene_output``: List of most variable genes
