...
plot_PC <- function(gbmData){
  title <- "PCA"
  reduction <- "pca"
  p <- DimPlot(object = gbmData, reduction.use = reduction, pt.size = 0.5, do.return = TRUE, group.by = "lib_id")
...
run_PCA <- function(gbmData){
  ...
  p <- plot_PC(gbmData)
  print(p)
  if (visr.isGUI()){switchPlotToScreen()
  print(p)
  switchPlotToReport()}
...
visr.param.jackstrawPlot_ncol <- 3
...
run_tSNE <- function(gbmData){
  ...
  gbmData <- RunTSNE(object = gbmData, dims.use = 1:num_pc_to_use, do.fast = T) #seed.use = 101) #change seed if Rtsne failed
  p <- plot_tsne(gbmData)
...
plot_tsne <- function(gbmData){
  ...
  if (is.null(gbmData@meta.data$group)){
    if (length(levels(gbmData@ident))>1){group <- "ident"}
    else {group <- "lib_id"}
  } else {
    if (length(levels(gbmData@ident))>1){group <- "ident"}
    else {group <- "group"}
